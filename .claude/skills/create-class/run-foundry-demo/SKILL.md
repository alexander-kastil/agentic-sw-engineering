...
description: >-
  Guide the user through running a Foundry demo in demos/02-agents. Prepares
...
Look for `provision.azcli` in the demo's module folder (e.g., `demos/02-agents/04-foundry-iq/provision.azcli`). If present, run it in Bash before doing anything else. It creates all required Azure resources and updates `.claude/deploy.config`.
...
Identify the target `*-py/` folder under `demos/02-agents/`. If not clear from context, ask.
...
After resolving any issues, update the `readme.md` in the parent folder of the demo (e.g. `demos/02-agents/04-foundry-iq/readme.md`) with:
