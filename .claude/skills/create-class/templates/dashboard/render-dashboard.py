...
DEMO_FILE_RE = re.compile(r"^(?:\d{2}-demo-|demo-).*\.md$", re.IGNORECASE)
...
def list_module_demos(module_dir, module_path, pr_map):
    if not module_dir or not module_dir.exists():
        return []
    scan_dirs = [(module_dir, module_path)]
    demos_sub = module_dir / "demos"
    if demos_sub.is_dir():
        scan_dirs.append((demos_sub, f"{module_path}/demos"))
    for child in sorted(module_dir.iterdir()):
        if child.is_dir() and len(child.name) >= 2 and child.name[:2].isdigit():
            scan_dirs.append((child, f"{module_path}/{child.name}"))
    demos = []
    for base_dir, base_rel in scan_dirs:
