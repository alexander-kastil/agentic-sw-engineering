---
name: use-claude-app (reference for evaluate-guide)
description: >-
...
Automate Claude Desktop (or any desktop app) from Claude Code using Midscene visual AI. This skill covers full setup, correct model configuration, Windows-specific fixes, and reliable interaction patterns.
...
The standard window size for Midscene tasks is **1400 × 900 px**. On 4K monitors, full-screen windows contain too many pixels for Midscene's element-detection model to locate UI elements reliably. Resizing to 1400 px wide keeps the target app visible and the AI accurate.
...
[WinCtrl]::MoveWindow($proc.MainWindowHandle, 100, 50, 1400, 900, $true)   # default: 1400 wide
...
- `.claude/skills/evaluate-guide/references/chat-context.md` — Chat mode and Chat project navigation tables
- `.claude/skills/evaluate-guide/references/project-context.md` — Chat project and Cowork project navigation tables
- `.claude/skills/evaluate-guide/references/cowork-context.md` — Cowork mode navigation table
