...
| Path | Purpose |
|------|---------|
| `../templates/dashboard/dashboard.json` | Config plus generated state: assignee, issue numbers, project meta (hand-maintained) plus `theory`, `demos`, `labs`, `slides` status fields written by `evaluate.py`. |
| `../templates/dashboard/evaluate.py` | Inspects the repo and writes status fields into `dashboard.json`. Automated via GitHub Actions; also runnable locally. |
| `../templates/dashboard/render-dashboard.py` | Reads `dashboard.json`, injects data inline into `index.html` between `<!-- DATA:START -->` and `<!-- DATA:END -->`. |
| `../templates/dashboard/index.html`, `../templates/dashboard/styles.css`, `../templates/dashboard/app.js` | The static web app. Self-contained; opens directly from disk. |
| `../templates/dashboard/test_evaluate.py` | Unit tests for the evaluator. |
| `../templates/dashboard/collect_contributors.py`, `../templates/dashboard/contributors-override.json` | Contributor collection helper and override list. |
| `../templates/dashboard/update-class-dashboard.yml` | The GitHub Pages deployment workflow. |
| `class-module-evaluation.md` | Discovery rules the evaluator applies per component. |
...
A module's overall status is the worst state across its components. See `class-module-evaluation.md` for the discovery rules the evaluator applies per component (theory, code demos, labs, slides).
...
3. For each file in `../templates/dashboard/` (relative to this reference file), except `update-class-dashboard.yml`:
...
- When a new component is added to the model (e.g. a fifth column beyond Theory, Demos, Labs, Slides), update `class-module-evaluation.md`.
...
1. Read `../templates/dashboard/update-class-dashboard.yml` (relative to this reference file).
