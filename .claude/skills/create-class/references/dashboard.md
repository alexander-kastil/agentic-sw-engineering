---
name: dashboard
description: Scaffold, evaluate, render, update, show, test, and publish the class progress dashboard.
---

# Dashboard

This leaf is loaded by the `create-class` master skill. It merges six former operations into one document with a sub-verb section each: `setup`, `evaluate` and `render`, `update`, `show`, `test`, and `publish`. Each sub-verb is self-contained; pick the one that matches the request.

## Files

| File | Purpose |
|------|---------|
| `dashboard/dashboard.json` | Config plus generated state: assignee, issue numbers, project meta (hand-maintained) and `theory`, `demos`, `labs`, `slides` status fields written by `evaluate.py`. |
| `dashboard/evaluate.py` | Inspects the repo and writes status fields into `dashboard.json`. Automated via GitHub Actions; also runnable locally. |
| `dashboard/render-dashboard.py` | Reads `dashboard.json`, injects data inline into `dashboard/index.html` between `<!-- DATA:START -->` and `<!-- DATA:END -->`. |
| `references/class-module-evaluation.md` | Discovery rules used by the evaluator. |

All dashboard files live in `dashboard/` at the repo root: source data, scripts, and the static web app. `index.html` is self-contained, the rendered JSON is injected inline so it opens directly from disk without a server.
...
1. Locate the plugin root (directory containing `.claude-plugin/plugin.json`).

2. Create `dashboard/` at the repo root if it does not exist.

3. For each file in `templates/dashboard/` (relative to the create-class skill root):
...
1. Read `templates/update-class-dashboard.yml` (relative to the create-class skill root).
