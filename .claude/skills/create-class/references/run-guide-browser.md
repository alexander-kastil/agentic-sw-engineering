...
External vision automation with a cheap Chinese VL model (Qwen3-VL on DeepInfra) driving the app from screenshots. Full setup, model configuration, window management, and interaction patterns live in `use-claude-app.md`. This is the required backend for Cowork and other desktop-only surfaces.
...
- `chat-context.md`, Chat mode navigation (new chat, input, model picker, attach, sidebar)
- `project-context.md`, Chat project and Cowork project navigation (right panel, Instructions, Files, Context folder)
- `cowork-context.md`, Cowork mode navigation (task input, Work in a project, Ask, model picker); Midscene backend only
- `attach-files.md`, attaching files to a Project versus a single chat message

For the Midscene backend, also read `use-claude-app.md` for environment setup, window positioning, connect and health-check, and the clipboard paste pattern.
...
- **Midscene:** follow `use-claude-app.md` to set the model env vars and PATH, position the Claude Desktop window (1400 wide, full height), then `npx midscene-computer connect`. If the health check fails, stop and report `SKIPPED: <filename>: Midscene health check failed`.
...
## Verify the write path, not just the read-only panels

Reading a UI and successfully submitting to it are different claims, and only the second one validates a guide step. Opening panels, dialogs, and dropdowns is cheap reconnaissance that catches wrong labels and missing controls; it proves nothing about whether an upload parses, a form saves, or a run starts.

**Every guide step that submits, uploads, or saves gets executed once against the live product before the guide ships.** A guide whose read-only steps were verified and whose upload step was not is the guide that fails in front of a class. In one session the Evaluate tab's panels were inspected thoroughly, the companion CSV was never actually uploaded, and the learner hit a hard format error on the first try; performing the upload cost one tool call.

Where a submit creates state you do not want to leave behind, drive it to the point of confirmation and then abandon it: fill a draft and read the success banner without pressing Save, or delete the artifact in teardown. Note in the report which write steps were executed and which were only described.

## Companion assets bound to a product-defined format

When a guide ships a file the learner uploads (a CSV test set, an import template, a config file), the product's own template is the contract, not the documentation. Obtain it before authoring the asset.

Writing "download the template and confirm the columns" into the guide is not verification. It reads like diligence while shipping an unverified file, and it moves your unfinished work onto the learner.

If the download is generated client-side (a control with no `href` that builds a Blob), read it without saving a file:
...
Read the captured text in line slices rather than as one string; a large blob returned whole can trip a content filter. Then author the asset against the real headers, upload it, and confirm the product accepts it.
...
- A guide step that uploads or submits is not verified until it was executed and the product's success state was observed.
