...
Demo guide files normally sit in their owning topic folder (`<root>/NN-topic/demo-*.md`); legacy layouts keep them at the module root (`<root>/demo-*.md`) or in a `demos/` subfolder (`<root>/demos/demo-*.md`). The evaluator scans all three locations.
