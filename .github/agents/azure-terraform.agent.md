...
- Use execute tool to verify or create the folder (e.g., `mkdir -p <outputBasePath>`), then proceed.

## Testing & validation

- Use execute tool to run: `terraform init` (initialize and download providers/modules)
- Use execute tool to run: `terraform validate` (validate syntax and configuration)
- Use execute tool to run: `terraform fmt` (after creating or editing files to ensure style consistency)
- Offer to use execute tool to run: `terraform plan` (preview changes - **required before apply**). Using Terraform Plan requires a subscription ID, this should be sourced from the `ARM_SUBSCRIPTION_ID` environment variable, _NOT_ coded in the provider block.
...
- **tflint**: `tflint --init && tflint` (suggest for advanced validation after functional changes done, validate passes, and code hygiene edits are complete). Add `.tflint.hcl` if not present.
...
- After any command check if the command failed, diagnose why and retry
