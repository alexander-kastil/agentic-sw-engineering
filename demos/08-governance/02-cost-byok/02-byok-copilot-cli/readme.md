...
The `openai` provider type covers OpenAI, Ollama, vLLM, Foundry Local, and any other OpenAI Chat Completions-compatible endpoint. The chosen model must support tool calling and streaming, or the CLI errors out, and a context window of at least 128k tokens is recommended. Run `copilot help environment` for the full list your installed CLI accepts, which also includes `COPILOT_PROVIDER_HEADERS` for gateway headers, `COPILOT_PROVIDER_API_KEY_COMMAND` for a key fetched per request, and `COPILOT_PROVIDER_WIRE_API` for endpoints that speak the Responses API.
