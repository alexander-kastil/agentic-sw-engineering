...
Two newer readouts close the loop after the fact. Hovering the footer of a chat response shows a per-model breakdown of input, cached input, and output tokens (1.135), which is the only place the cached-token driver above becomes directly observable. The Copilot status menu shows aggregate credit usage for the current billing cycle on Business and Enterprise plans (1.130), so the month-to-date number is one click away rather than a billing-portal trip.
...
| Chat response footer | Per-model input, cached input, and output token counts on hover (1.135) |
| Copilot status menu | Aggregate credit usage for the current billing cycle, on Business and Enterprise (1.130) |
...
6. Hover the footer of one of those responses and read the input, cached input, and output token counts. Compare a first turn against a follow-up in the same session to see caching take effect.
7. Open the Copilot status menu and read the aggregate credit usage for the current billing cycle.
...
- [VS Code 1.130 release notes](https://code.visualstudio.com/updates/v1_130) - aggregate AI credit usage in the Copilot status menu
- [VS Code 1.135 release notes](https://code.visualstudio.com/updates/v1_135) - detailed chat turn usage with per-model token breakdown
