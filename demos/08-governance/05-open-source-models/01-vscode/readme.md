...
Reach for the extension when your endpoint is genuinely arbitrary, not before. Since VS Code 1.133 the picker shows separate Anthropic and Copilot groups natively and lets you switch providers between turns without reconfiguring the agent host: models under Anthropic bill your API key, models under Copilot draw on your subscription. BYOK models also work in the Agents window under the Copilot harness (1.129), and `chat.agentHost.allowSignedOutWhenUsable` (1.133) opens that window with a Claude API key configured and no GitHub sign-in at all.
...
- [VS Code 1.129 release notes](https://code.visualstudio.com/updates/v1_129) - BYOK models in the Agents window under the Copilot harness
- [VS Code 1.133 release notes](https://code.visualstudio.com/updates/v1_133) - Anthropic and Copilot model groups, per-turn provider switching, and signed-out access
