...
| **[Extending the CLI with MCP Servers & Skills](./04-mcp-skills/)** | Connect the CLI to external systems with `/mcp` and load reusable Agent Skills with `/skills`, then bundle both as an Agent Plugin so the same capabilities travel to the editor. |
