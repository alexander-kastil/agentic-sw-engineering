...
| Topic | Description |
|-------|-------------|
| **[GitHub Copilot SDK](./01-sdk/)** | The SDK fundamentals: install for your stack, create a `CopilotClient`, open a session, and register custom tools the agent invokes on its own, all on the same runtime that powers the Copilot CLI and the editor's agent host, in Technical Preview for Python, TypeScript, Go, and .NET. |
