...
A four-day journey into agentic software engineering with GitHub Copilot, and how AI-powered tools change coding productivity and architectural decisions. Software engineers, architects, and technical leaders get hands-on experience, with samples in C# and Python and sometimes other stacks.

Your journey starts with the fundamentals: prompting, inline suggestions, code review on pull requests, and choosing a model for the task. Agent Mode arrives early, so a local agent is doing multi-step work for you before the first module ends.

Next you assemble the GitHub Copilot Harness. Instructions and prompt files come first, then the Model Context Protocol, from the MCP Registry to your own servers and MCP Apps. Skills, custom agents, memory, and hooks follow, and Agent Plugins 1.0 packs them into one install.

In Implementing Agentic Coding you put agents to work. A local agent runs in Agent Mode, heavy jobs go to cloud agents, and an orchestrator runs expert subagents in parallel. Browser tools let an agent check its own web changes, a migration to the Microsoft Agent Framework shows an agent modernizing real code, and `/research` and `/rubber-duck` add cited research and a second opinion.

Agent Sessions covers the machinery under those runs. You work across projects in the Agents window, see how the Agent Host Protocol keeps session state on a long-lived host, and drive remote sessions over SSH and dev tunnels. Groups, background sends, and one-click banners keep many sessions moving at once.

The GitHub Copilot CLI brings the agent to the command line, with Autopilot for multi-step work. An HR document case over the Work IQ and SharePoint MCP servers shows the payoff, and GitHub Agentic Workflows turn that job into a scheduled run that opens pull requests.

The GitHub Copilot SDK puts the same agent inside your own applications. You create sessions and custom tools while Copilot plans and executes, compose several agents into one system, and deploy one to Azure.

The GitHub Copilot app is the desktop agents view for macOS, Windows, and Linux. A session starts from an issue, a prompt, or a pull request, and you steer it without stopping it. The validation loop carries the work from diff to merge, and automations turn skills and prompts into scheduled runs.

Agentic DevOps applies all of this to infrastructure as code with the Azure CLI, Bicep, and Terraform, and to CI/CD with GitHub Actions. The quality loop closes here too, with agent-generated tests and documentation drawn in Mermaid.

Governance, Cost and Observability is written for architects, team leads, and managers: what an agent may do on its own, what each run costs under usage-based credits, and how bring your own key routes work to the provider you choose. Enterprise controls, OpenTelemetry traces in Azure Managed Grafana, and the compliance duties of the software your agents ship complete the picture.

The course closes with Plan, Specify, Deliver. An agent harvests the current state of a brownfield system, you plan the change with agents, and GitHub Spec Kit takes a product feature from specification to working code. You leave able to design AI-assisted solutions that speed up delivery and make your team stronger.
...
- [Deep Research with /research (optional)](03-agentic-coding/06-research/)
- [A Second Opinion with /rubber-duck (optional)](03-agentic-coding/07-rubber-duck/)
