...
Model selection for a subagent resolves by exact CAPI model ID first and falls back to the model family only when that ID is unavailable (1.131). Pin the exact ID in an `.agent.md` when the expert's behavior depends on one specific model rather than on a family.
...
| Live status | The session in front of you | Model, elapsed time, and current tool call in the Agents window |
...
7. In the Agents window, read each running subagent's model, elapsed time, and current tool call without opening its conversation, and watch the Subagents activity pill above the chat input change as the phase progresses.
...
- [VS Code 1.131 release notes](https://code.visualstudio.com/updates/v1_131) - subagent model, elapsed time, and current tool call in the Agents window, and CAPI ID model resolution
- [VS Code 1.132 release notes](https://code.visualstudio.com/updates/v1_132) - session activity pills for Changes, Previews, Subagents, and Browsers
