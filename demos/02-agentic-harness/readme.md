...
| **[Model Context Protocol & MCP Registry](./03-mcp/)** | MCP from both ends, in three parts. [Basics](./03-mcp/01-basics/) covers discovering and configuring servers, local executables (npx, stdio) and remote HTTP endpoints, and workspace versus global `mcp.json`. [Implementing](./03-mcp/02-mcp-server/) covers writing your own servers in Python and C#, transports, state, and the MCP Inspector. [MCP Apps](./03-mcp/03-mcp-apps/) covers serving an interactive `ui://` UI that renders inline in chat. |
...
| **[Agent Plugins](./06-plugins/)** | Prepackaged bundles of chat customizations (skills, commands, agents, MCP servers, and hooks) under the Agent Plugins 1.0 open standard. Covers the manifest schema, the portable `skills/` and `mcp.json` core versus the `com.github.copilot/` directory, and loading a local plugin with `chat.pluginLocations`. Ships [demo-quality-plugin](./06-plugins/demo-quality-plugin/), a working plugin with a skill, an MCP server, an agent, and a hook. |
...
| **[Agent Interop](./09-agent-interop/)** (optional) | `AGENTS.md` as the harness-agnostic instructions file, how it coexists with the Copilot-specific formats, and which rule belongs where. Covers the VS Code Session Target picker (Copilot, Claude, Codex, Cloud) and the Agent Host that keeps those sessions alive beyond the editor window. |
