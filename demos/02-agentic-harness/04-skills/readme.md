...
description: .NET 10 and C# 14 conventions, routed to short task-based references. Covers naming and file structure, dependency injection and service lifetimes, async patterns, EF Core and SQL delta scripts, xUnit testing, and MSAL auth. Read the ONE matching reference; never load them all. Use before writing any .NET code in this repository, including when the request never says "convention", such as "add an endpoint to the HR API" or "this field arrives undefined in the SPA".
license: MIT
metadata:
...
> Note: `description: Helps with .NET code.` is the most common defect in a skill that "does not work". The instructions were fine; the model never got far enough to read them.
...
Most of the value in a repository sits in the left column. The knowledge a team keeps re-explaining is judgement, not output: which pattern this codebase uses, which one it banned two years ago, and the three surprises nobody wrote down. That is the kind of skill the rest of this topic builds first.

## A Domain-Knowledge Skill End to End

[.github/skills/dotnet-conventions](/.github/skills/dotnet-conventions/) is the pattern in full. It carries everything this estate has decided about .NET 10 and C# 14: naming and file layout, service lifetimes, the JSON casing contract with SPA clients, EF Core, xUnit, MSAL auth, MCP servers, health checks. None of that is derivable from the code in front of the model, and all of it otherwise gets re-explained in every chat.

The body holds no conventions of its own. It is a router: a set of "You want to... / Read" tables that send the request to exactly one file under `references/`, and it says so in its opening lines so the model does not helpfully read five.

```markdown
| You want to... | Read |
|---|---|
| Inject dependencies, choose Singleton/Scoped/Transient, fix a captive DbContext | [`di-and-lifetimes`](references/di-and-lifetimes.md) |
| Fix a field arriving `undefined` in a SPA after a 200 response | [`json-casing-contract`](references/json-casing-contract.md) |
...
| Ship a schema change as a hand-written SQL delta script, where EF migrations are banned | [`sql-delta-scripts`](references/sql-delta-scripts.md) |
```

The router exists because of the budget. Thirty-two reference files are far past the 5000-token instruction limit, while the router itself costs a few hundred tokens and each leaf is short enough to read whole. The model pays for the table of contents on every .NET turn and for one chapter on the turn that needs it.

```mermaid
flowchart LR
    P["Add an endpoint<br/>to the HR API"] --> S["SKILL.md router<br/>loads"]
    S --> M{"Which row<br/>matches?"}
    M -->|"Controllers"| A["dotnet-controllers.md"]
    M -->|"EF Core"| B["dotnet-efcore.md"]
    M -->|"Auth"| C["dotnet-msal-auth.md"]
```

Two details in that file are worth copying as habits. The router ends with a section titled "The one rule that belongs here", holding the single prohibition that applies to every task regardless of which leaf loads: never hand-edit a `.csproj` to add or remove a package, always use the dotnet CLI. A rule that has to survive routing has to live above the routing.

The second is where corrections go. Every time you tell the agent it got something wrong on a real run, that correction belongs in the matching leaf, under a `Gotchas` heading, phrased as the fact that defied a reasonable assumption. A skill that does not grow this way is a document; one that does is what the next engineer would otherwise learn by breaking staging.

## When the Output Must Be Exact: a Deterministic Skill
...
This is also where the optional `compatibility` field earns its place. `Requires uv (or pipx) to run the bundled Python script` tells the agent what has to exist before the skill can do anything, which an instructions-only skill never needs to say.
...
The `Gotchas` section at the bottom of that `SKILL.md` holds the three facts that defy a reasonable assumption: duplicate slugs overwrite silently, a long URL at box size 10 exceeds 1000 pixels per side, and the model must never try to draw the matrix itself. Same habit as the conventions skill, different failure mode.
...
An orchestration skill is graded the same way, against artifacts rather than prose. For `dotnet-conventions` the assertions read "the controller uses the CRUD return shape from the reference", "the package was added with the dotnet CLI", and "no EF migration was created", each confirmable from the diff alone.
...
```bash
skills-ref validate ./.github/skills/dotnet-conventions
```
...
| [create-pptx](/.github/skills/create-pptx/) | Deterministic | `Create a PowerPoint about GitHub Actions best practices and validate the design` |

## Hands-On Demo: Route a Request Through dotnet-conventions

1. Confirm the setting `chat.useAgentSkills` is `true` in VS Code, then reload the window so the skills directory is rescanned.
2. Open Copilot Chat in agent mode with [src/hr-mcp-server](/src/hr-mcp-server/) in the workspace and ask: `Add a GET endpoint that returns one employee by id`. Expected result: the skill loads without you naming it, because the description covers a request that never says "convention".
3. Watch which files it opens. Expected result: `SKILL.md` plus `references/dotnet-controllers.md`, and nothing else from `references/`.
4. Ask a second question in the same chat: `Why would a field come back undefined in the Angular client after a 200?`. Expected result: it now opens `references/json-casing-contract.md`, which proves the router picks per request rather than per session.
5. Ask it to add a NuGet package. Expected result: it runs `dotnet add package` instead of editing the `.csproj`, because that rule sits in the router body above the routing tables.
6. Ask `What does this repository decide about EF migrations?`. Expected result: it answers from `references/sql-delta-scripts.md` and does not propose `dotnet ef migrations add`.

If step 3 shows the agent reading four reference files, the router line telling it to read exactly one is too weak. That is a body problem rather than a description problem, and it is the cheapest kind to fix.

## Hands-On Demo: Run the qr-batch Skill

1. In Copilot Chat, type `/qr-batch`. Expected result: the skill appears in the slash command list, which proves the frontmatter parsed and the folder name matches `name`.
2. Instead of the slash command, ask in plain language: `I need scannable images for the class repo and the integrations.at homepage, put them in ./out`. Expected result: the skill loads without you naming it, because the description covers the phrasing.
3. Watch what the agent asks for. Expected result: it asks nothing about box size or error correction, because both have defaults, and it uses the output directory you already gave.
4. Confirm it runs the dry run first and shows you the planned file list before writing. Expected result: JSON with `"dry_run": true` and two entries.
5. Approve the real run and open `out/class-repo.png`. Expected result: a QR code that resolves to the repository when scanned with a phone.

Run step 2 again with the phrase `generate a barcode for SKU 44812`. The skill must not load. If it does, the description is too broad and you have found your first real eval failure.

## Demo: Write and Optimize Your Own Skill
...
| Command | Usage |
|---------|-------|
| `/explain` | Explain why a skill did not load for a given prompt, or which reference the router would pick for a request |
...
A `user-invocable` skill also appears as its own slash command, so `/dotnet-conventions` loads the router directly and bypasses description matching entirely.
