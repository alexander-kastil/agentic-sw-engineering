...
To benefit, keep the cacheable part stable and up front: put instructions and big reference files first, and let the volatile part (your latest question, the current selection) come last. Editing or reordering early context invalidates the cache, so a small change near the top can force the whole prefix to be reprocessed. Cached tokens are also billed at a lower rate, so a stable prefix is a budget decision as much as a quality one, which ties directly to the cost model in the [Governance module](../../09-governance/02-cost/).
