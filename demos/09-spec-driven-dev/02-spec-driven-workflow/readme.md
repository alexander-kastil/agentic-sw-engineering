...
Beyond this intro, Spec Kit adds `/speckit-clarify` for gap analysis, `/speckit-analyze` for cross-artifact consistency, `/speckit-checklist` for quality validation, and `/speckit-taskstoissues` for pushing the task list to GitHub issues. Reach for them once the basic loop is familiar.

The workflow runs forward through those six commands, but it is not one-way. When requirements change, go back to the specification, regenerate the downstream artifacts, and let the change propagate instead of patching it into code where nobody can trace it.
