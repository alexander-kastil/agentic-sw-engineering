...
Expected: typing in either field updates the other, negative values work, and entering `abc` shows "Enter a valid number" without clearing the other field. Values display with one decimal place, so 0 C reads as `32.0` and 100 C as `212.0`. A converter that fails here usually means the two agents worked from different contracts, not that either wrote bad code.

A reference solution that passes every check above is checked in at [orchestration-solution/](orchestration-solution/readme.md). Compare your run against it; do not copy it into `src/scratch/delegation-test/` before the run, or the trace you are auditing is not your own.
