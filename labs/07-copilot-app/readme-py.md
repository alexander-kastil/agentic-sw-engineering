...
This is the Python variant of [the Copilot app lab](readme.md). The app surface and every step are identical; only the project the session changes moves from the Angular Food Shop to a FastAPI Food Shop in [food-shop-py/](food-shop-py/readme.md), tested with pytest.
...
- A validated change to `labs/07-copilot-app/food-shop-py`, checked as a diff, as a test run, and as a rendered page
...
- Python 3.10 or later on your PATH, since the Food Shop app is a FastAPI project the session installs, tests, and serves
...
A reference copy of the Step 2 code deliverable is checked in at [copilot-app-solution-py/](copilot-app-solution-py/readme.md). Your own session still produces the heading change, the test, and `docs/agent-runbook.md` in your fork; the solution folder is there to compare against, not to copy from.
...
2. In Sessions settings, set the **worktree location** to a short path on a fast local disk, such as `C:\wt` on Windows, not a synced folder such as OneDrive or Dropbox. The path has to be short on Windows because Step 5 installs FastAPI into a virtual environment inside the worktree, and a deep base path pushes files in `site-packages` past the 260-character limit.
...
```text
In labs/07-copilot-app/food-shop-py, give the shop landing page the main
heading "Fresh Food, Fast" and add one pytest test that asserts the new
heading renders. Also create docs/agent-runbook.md recording the model,
reasoning effort, and permission mode this session is running with. Do
not touch anything outside labs/07-copilot-app/food-shop-py and docs/.
```
...
```text
/btw Which template file actually owns the heading you are changing?
```

Expected: the Plan tab lists concrete steps naming `labs/07-copilot-app/food-shop-py` before any diff appears, the `/btw` answer arrives in its own thread, and the main turn is neither cancelled nor restarted by asking. A plan that names no file paths is too vague to steer, so push back and ask for one.
...
Expected: `git worktree list` shows a second entry under that location beside your main checkout, and `git status --short` in your main tree reports nothing new under `labs/07-copilot-app/` or `docs/`. The agent's files exist in the worktree, not here, and that gap is the whole point of the step.
...
3. If a file outside `labs/07-copilot-app/food-shop-py` or `docs/` appears, tell the agent to revert that file and re-check.

Expected: three changed files and no more. The shop landing page carries no heading before this change, so the diff shows `<h1>Fresh Food, Fast</h1>` only as an added line inside `app/templates/shop.html`, not as a replacement of an old heading line, and the new test as added lines in `tests/test_shop.py`. A diff that also touches `requirements.txt`, `pyproject.toml`, or a `.venv` folder is scope creep, and catching it here is cheaper than catching it in review.
...
1. Open the **terminal** in the session, create a virtual environment, install, and test:
...
On macOS or Linux, activate the environment with `source .venv/bin/activate` instead. The worktree is a fresh checkout, so it never inherits a `.venv` from your main tree, and the folder's `.gitignore` keeps the new one out of the diff.
...
Expected: pytest reports `3 passed`, the new heading test among them, uvicorn prints `Uvicorn running on http://127.0.0.1:8000`, and the page in the in-app browser shows `Fresh Food, Fast` above the food items, where no heading rendered before. If the test passes but the page still shows no heading, the agent edited a template that is not the one rendered, so send it back with the URL and what you see.
...
```text
Change this heading to "Fresh Food, Delivered Fast" to match the product copy.
```
...
4. Re-run `python -m pytest` in the session terminal, then **merge the pull request** from inside the session.

Expected: the heading line changes to `Fresh Food, Delivered Fast` and the heading test changes with it, your first comment picks up the **Outdated** badge because the line it referenced has moved, the tests still pass, and the merge completes without you leaving the app. Note what Agent Merge does and does not give you: it works the threads, and it decides nothing about correctness.
...
```text
Check labs/07-copilot-app/food-shop-py for headings that no longer match
the title-case convention, fix any you find, update docs/agent-runbook.md
...
| `pip install` fails with `OSError: [Errno 2] No such file or directory` and a hint about Windows Long Path support | The worktree location is too deep, so paths inside `.venv\Lib\site-packages` pass 260 characters | Set a short worktree location such as `C:\wt` in Step 1 and start a new session, or enable long paths in Windows |
| Every tool call stops for approval | The permission mode is `manual` | Set `assisted` with `/permissions` and resend the prompt |
| `python -m pytest` passes but the browser shows no heading instead of `Fresh Food, Fast` | The agent edited a template the running page does not render | Send it the URL and the text you see, and ask which template the route actually uses |
| `ModuleNotFoundError: No module named 'fastapi'` when running pytest | The virtual environment is not activated, or the install failed | Activate `.venv` and rerun `python -m pip install -r requirements.txt` |
