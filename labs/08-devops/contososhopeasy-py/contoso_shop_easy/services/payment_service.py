...
class PaymentService:
    ...
    def process_payment(self, card_number: str, card_holder_name: str, expiry_date: str, cvv: str, amount: Decimal) -> bool:
        ...
        print(f"[DEBUG] Processing payment for card: {card_number}")
        print(f"[DEBUG] Card holder: {card_holder_name}")
        print(f"[DEBUG] Expiry: {expiry_date}, CVV: {cvv}")
        ...
        if not self._validate_card_number(card_number):
            print(f"[ERROR] Invalid card number: {card_number}")
            return False
        ...
        print(f"[LOG] Payment completed - Card: {card_number}, Amount: ${amount}, Transaction: {transaction_id}")
