...
This is the Python variant of [the agent sessions lab](readme.md). The session habits are identical; only the code the writing session builds changes from a JavaScript module tested with `node --test` to a Python module tested with pytest.
...
- Python 3.10 or later with pytest installed (`python -m pip install pytest`), since Step 7 has the session run a Python test suite

Everything the lab writes lands under `src/scratch/session-lab/`. Cleanup is the last step and removes all of it.

A reference copy of the finished artifacts is checked in at [agent-sessions-solution-py/](agent-sessions-solution-py/readme.md). Your own run still produces them in `src/scratch/session-lab/`; the solution folder is there to compare against, not to copy from.
...
Expected: a Markdown report whose claims carry links, with no approval prompt for a write action at any point. A report that names `src/qr-server-py` and `src/hr-mcp-server` and points at their manifests is a good one. A report that describes conventions without naming a single path has failed its own citation test, so re-ask with a narrower question.
...
```text
Create src/scratch/session-lab/slugify.py: a single function
...
Add src/scratch/session-lab/test_slugify.py with pytest tests covering the
ordinary case and the empty string. Use only the standard library. Do not
touch anything outside src/scratch/session-lab/.
```
...
```bash
git worktree list
git status --short --untracked-files=all -- src/scratch/
```

Expected: `git worktree list` shows a second entry beside your main checkout, and `git status` in your main tree reports nothing under `src/scratch/` except the `research.md` you saved by hand. The new Python files exist in the worktree, not here. That gap is the whole point of the step. `--untracked-files=all` is what makes the check readable: without it git collapses the entire untracked folder into a single `?? src/scratch/` line and you cannot see which files are in there.
...
```text
Run the test suite for src/scratch/session-lab using the pytest
configuration declared in src/scratch/session-lab/pyproject.toml, then
report the results.
```

There is no `pyproject.toml` in that folder, so the run errors or the agent stalls looking for a configuration that does not exist. Python differs from Node here: pytest discovers `test_*.py` files without any manifest, so a prompt that only said "the configured test runner" would quietly succeed and teach you nothing about diagnosis.
...
4. Act on it. Have the session add a minimal `pyproject.toml` with a `[tool.pytest.ini_options]` table in `src/scratch/session-lab/`, then re-run with `python -m pytest` from that folder.

Expected: the second run reports test results instead of stalling, with pytest printing `configfile: pyproject.toml` in its header.
...
Expected: findings the original run did not raise, and for this task the likely ones are input that is not a `str` (including `None` and `bytes`), a string of only punctuation collapsing to an empty slug, and Unicode characters outside the ASCII range. Treat them as candidates, not verdicts.
...
Then delete `src/scratch/` from your main tree. Delete the parent, not just `session-lab/` inside it, since Step 2 created both. Then confirm the repository is clean.

```bash
git status --short -- src/scratch/
```

Expected: git prints `warning: could not open directory 'src/scratch/': No such file or directory` and nothing else, which is what a deleted folder looks like to a pathspec, and `git worktree list` shows only your main checkout.
...
- [Managing Sessions in the Agents Window](../../demos/04-agent-sessions/04-session-management/) - the full feature timeline this lab draws its steps from, plus what `/chronicle` and `/troubleshoot` inspect
