...
- Node.js 20 or later on your PATH, since Step 7 has the session run a JavaScript test suite

Everything the lab writes lands under `src/scratch/session-lab/`. Cleanup is the last step and removes all of it.

A reference copy of the finished artifacts is checked in at [agent-sessions-solution/](agent-sessions-solution/readme.md). Your own run still produces them in `src/scratch/session-lab/`; the solution folder is there to compare against, not to copy from.
...
Expected: `git worktree list` shows a second entry beside your main checkout, and `git status` in your main tree reports nothing under `src/scratch/` except the `research.md` you saved by hand. The new JavaScript files exist in the worktree, not here. That gap is the whole point of the step. `--untracked-files=all` is what makes the check readable: without it git collapses the entire untracked folder into a single `?? src/scratch/` line and you cannot see which files are in there.
