# Test Agent Delegation with One Prompt

A capable orchestrator delegates in tool calls, not in prose. This lab runs a single prompt that forces the three delegation behaviors worth verifying: a Planner call before any work, a parallel split across Coder and Frontend with zero file overlap, and real agent invocations visible in the chat trace. By the end you hold a working temperature converter in `src/scratch/delegation-test/` and a pass-or-fail audit you can run in under a minute.
...
The prompt embeds its own grading criteria, so the outcome is checkable instead of vibes:
...
| Coder and Frontend in one parallel phase | Chat trace | Serial delegation, or one agent touching the other's files |
...
The failure to spot: the model narrates the delegation instead of emitting the tool call. The summary reads fine; the trace is empty.
...
It is shaped to force every delegation behavior at once: non-trivial enough that the orchestrator must call Planner first, three deliverables that split cleanly across Coder (logic + page wiring) and Frontend (markup/styling) in one parallel phase with no file overlap, and no e2e wording so a team-playwright call would be a rule violation you can spot.

What a pass looks like: an ## Execution Plan block with phases, then actual agent tool invocations in the chat trace (not prose saying "I would delegate to Coder"), then files on disk in src/scratch/delegation-test/. The usual DeepSeek-via-shim failure is the middle one: it narrates the delegation instead of emitting the tool call.
```

## Verify the trace

The plan is only the beginning. The pass condition lives in the trace, so read it like a reviewer, not a reader.
...
4. Reject the run if the trace narrates delegation ("I would delegate to Coder") instead of invoking the agent. That is the standard failure mode.
...
When the trace passes, check the files it claims to have written:
...
Expected: typing in either field updates the other, negative values work, and entering `abc` shows "Enter a valid number" without clearing the other field.

## Troubleshooting

| Symptom | Likely cause | Fix |
|---------|--------------|-----|
| The response narrates delegation but the trace has no tool calls | The model is describing delegation instead of emitting it | Treat the run as a fail and re-run on a harness that emits real agent invocations |
| No Planner call before the build | The orchestrator judged the task too simple to plan | The prompt is non-trivial by construction; a plan phase is mandatory |
...
You ran one prompt that exercises the full delegation path: Planner first, a parallel Coder plus Frontend split with no file overlap, and a trace you can audit for real tool invocations. You can now:

- Spot prose delegation ("I would delegate to Coder") against real agent tool calls in a trace
- Verify a shared contract across files: element ids, function names, script order
...
Next: reuse the same prompt shape to audit delegation behavior in your own multi-agent workflows.
...
- [Creating custom agents for Copilot](https://docs.github.com/en/copilot/how-tos/copilot-on-github/customize-copilot/customize-cloud-agent/create-custom-agents) - how to configure the subagents that delegation routes to
