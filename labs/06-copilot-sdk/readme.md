...
A reference solution is checked in at [copilot-sdk-solution/](copilot-sdk-solution/readme.md). Your own run still produces the same files in your own ContosoShop clone; the solution folder is there to compare against, not to copy from.
...
1. Add the following `using` statement at the top of the file, after the existing `using` statements:
    ...
    > **NOTE**: The GitHub Copilot SDK's public types (`CopilotClient`, `CopilotSession`, `SessionConfig`, and related types) live in the `GitHub.Copilot` namespace, not `GitHub.Copilot.SDK`.
...
1. To create and register a CopilotClient singleton, add the following code:
    ...
    The `CopilotClient` manages the Copilot CLI process lifecycle. `CopilotClientOptions` has no `AutoStart` property; the CLI server is started explicitly in the next step by calling `StartAsync()`. `LogLevel` takes a `CopilotLogLevel` value (`None`, `Error`, `Warning`, `Info`, `Debug`, `All`), not a string.
...
1. Take a minute to review the session configuration code that you just added.
    ...
    - `OnPermissionRequest` auto-approves every tool call. Without it, the SDK's default permission gate silently blocks all four custom tools and the agent falls back to a generic "I'm unable to access order details right now due to a permission issue" answer instead of real data, because these tools are the app's own backend logic rather than an action that needs the customer's confirmation. `GitHub.Copilot.Rpc.PermissionDecision` is marked experimental by the SDK, so referencing it requires suppressing diagnostic `GHCP001`.
...
1. Take a minute to review the event handler code that you just added.
    ...
    The Copilot SDK uses an event-driven model for communication. `CopilotSession.On<T>` is a generic method, so the type argument `SessionEvent` (the common base type of `AssistantMessageEvent`, `SessionIdleEvent`, and `SessionErrorEvent`) must be given explicitly; the compiler cannot infer it from an untyped lambda parameter. The `session.On<SessionEvent>` method registers a callback that handles three event types:
